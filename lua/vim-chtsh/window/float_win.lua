...
function float_win.create_float_win(options)
    ...
    vim.keymap.set('n', 'q', vim.api.nvim_win_close(win_id, 1), { silent = true, buffer = bufnr })
    ...
    vim.cmd(string.format(
        "autocmd WinClosed,WinLeave <buffer=%s> ++nested ++once lua require('vim-chtsh.window').close_related_win(%s, %s)",
        bufnr,
        win_id,
        border.win_id
    ))
